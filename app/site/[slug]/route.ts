...
async function loadAndRender(site: any, page: string, supabase: any): Promise<string> {
  ...
  if (templateSlug === 'green-valley-industrial') {
    html = await renderGreenValleyPage(getContent, colors, fonts, manufacturers || [], sectionVisibility, siteId, site.site_name, displayProducts, isRealProducts, fmtPrice, availablePages, page, googleFontsUrl, supabase, '/', site.addons || []);
  } else if (templateSlug === 'vibe-dynamics') {
    html = await renderVibeDynamicsPage(getContent, colors, fonts, manufacturers || [], sectionVisibility, siteId, site.site_name, displayProducts, isRealProducts, fmtPrice, availablePages, page, googleFontsUrl, supabase, '/', site.addons || []);

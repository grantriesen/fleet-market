...
function weServicePage(siteId: string, gc: (k: string) => string, C: any, hasScheduler: boolean = false,
  ...
  <style>.we-scheduler-wrap > section > div > h2, .we-scheduler-wrap > section > div > p { display: none !important; }</style>
  ${hasScheduler ? `<div class="we-scheduler-wrap">${serviceBookingSection(siteId, C.accent, gc)}</div>` : `
